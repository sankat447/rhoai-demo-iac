...
#   VPC → ROSA HCP → IAM/IRSA → Aurora Serverless → S3 → EFS → ECR → Lambda
#
...
#
# Deployment order:
#   1. terraform apply → creates all platform infra (~25 min for ROSA)
#   2. scripts/post-apply.sh → runs init.sql, stores kubeconfig in SSM
#   3. ArgoCD bootstrap → deploys application layer from GitOps repo
# ─────────────────────────────────────────────────────────────────────────────

locals {
  # Computed values used across modules
  name    = "${var.project_name}-${var.environment}"  # e.g. rhoai-demo
...
# ─────────────────────────────────────────────────────────────────────────────
# MODULE 2: S3 — Bootstrap first (Terraform state + data lake buckets)
# ─────────────────────────────────────────────────────────────────────────────
module "s3" {
...
# ─────────────────────────────────────────────────────────────────────────────
# MODULE 4: IAM / IRSA — Pod-level AWS credential federation
# ─────────────────────────────────────────────────────────────────────────────
module "iam_irsa" {
