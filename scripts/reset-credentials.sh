#!/usr/bin/env bash
# =============================================================================
#  Complete Credential Reset & Re-authentication
#  Clears all cached tokens and performs fresh login
# =============================================================================
set -euo pipefail
...
echo -e "${BLUE}${BOLD}║  Complete Credential Reset & Re-authentication                       ║${RESET}"
echo -e "${BLUE}${BOLD}╚══════════════════════════════════════════════════════════════════════╝${RESET}"
echo ""

# Step 1: Clear AWS SSO cache
echo -e "${YELLOW}Step 1: Clear AWS SSO Cache${RESET}"
echo "Clearing AWS SSO cached credentials..."
rm -rf ~/.aws/sso/cache/* 2>/dev/null || true
echo -e "${GREEN}✓ AWS SSO cache cleared${RESET}"
echo ""

# Step 2: Clear ROSA cache
echo -e "${YELLOW}Step 2: Clear ROSA Cache${RESET}"
echo "Clearing ROSA/OCM cached credentials..."
rm -rf ~/.config/rosa/ 2>/dev/null || true
rm -rf ~/.ocm/ 2>/dev/null || true
echo -e "${GREEN}✓ ROSA cache cleared${RESET}"
echo ""

# Step 3: AWS SSO Login
echo -e "${YELLOW}Step 3: AWS SSO Authentication${RESET}"
...
aws sso login --profile rhoai-demo || {
  echo -e "${RED}✗ AWS SSO login failed${RESET}"
  exit 1
}
...
echo -e "${YELLOW}Step 4: Verify AWS Credentials${RESET}"
...
# Step 5: ROSA/OCM Login
echo -e "${YELLOW}Step 5: ROSA/OCM Authentication${RESET}"
echo "Logging in to Red Hat SSO (browser-based)..."
echo "A browser window will open. Complete the login and return here."
echo ""
rosa login --use-auth-code || {
  echo -e "${RED}✗ ROSA login failed${RESET}"
  echo "Troubleshooting:"
  echo "  1. Ensure you have a Red Hat account at https://console.redhat.com"
  echo "  2. Check your internet connection"
  echo "  3. Try again: rosa login"
  exit 1
}
...
# Step 6: Verify ROSA authentication
echo -e "${YELLOW}Step 6: Verify ROSA Authentication${RESET}"
WHOAMI=$(rosa whoami 2>&1)
OCM_EMAIL=$(echo "$WHOAMI" | grep "OCM Account Email" | awk '{print $NF}')
echo -e "${GREEN}✓ OCM Email: ${OCM_EMAIL}${RESET}"
echo ""

# Step 7: Export RHCS_TOKEN for Terraform
echo -e "${YELLOW}Step 7: Export RHCS_TOKEN for Terraform${RESET}"
...
# Step 8: Summary
echo -e "${BLUE}${BOLD}╔══════════════════════════════════════════════════════════════════════╗${RESET}"
echo -e "${BLUE}${BOLD}║  Credential Reset Complete ✓                                         ║${RESET}"
echo -e "${BLUE}${BOLD}╚══════════════════════════════════════════════════════════════════════╝${RESET}"
echo ""
echo -e "${GREEN}All credentials refreshed and ready to use!${RESET}"
echo ""
echo -e "${GREEN}You can now run:${RESET}"
echo -e "  ${YELLOW}./tests/test-ai-demo-stack-create.sh${RESET}"
